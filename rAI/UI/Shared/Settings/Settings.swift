...
struct Settings: View {
    ...
    private func save() {
        ...
        rAIService.shared.initEndpoint(url: ollamaUri, bearerToken: ollamaBearerToken)
        Task {
    ...
    private func checkServer() {
        Task {
            rAIService.shared.initEndpoint(url: ollamaUri)
            ollamaStatus = await rAIService.shared.reachable()
            try? await languageModelStore.loadModels()
        }
    }
