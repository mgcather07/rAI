...
// MARK: - Language Models
extension SwiftDataService {
    func fetchAgents() throws -> [AgentModelSD] {
        let sortDescriptor = SortDescriptor(\AgentModelSD.name)
        let fetchDescriptor = FetchDescriptor<AgentModelSD>(sortBy: [sortDescriptor])
        let models = try modelContext.fetch(fetchDescriptor)
        
        return models
    }
    
    func saveAgents(agents: [AgentModelSD]) throws {
        for agent in agents {
    ...
    func deleteAgents() throws {
        try modelContext.delete(model: AgentModelSD.self)
        try modelContext.saveChanges()
    }
}
